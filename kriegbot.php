...
$this->send("NOTICE ".$this->data['from']." :PING ".time()."");
break;
default:
if(!fnmatch("*.*.*", $this->data['from']) AND "Spacedock" != $this->data['from'] AND "anonymous.services" != $this->data['from'] AND "NickServ" != $this->data['from'] AND "HostServ" != $this->data['from'] AND "ChanServ" != $this->data['from'] AND "Shaun" != $this->data['from'])
{
...
function votekick(){
$this->database_connect();
$result = mysql_query("SELECT `id` FROM `bot_vote` WHERE`channel` = '".$this->serverchannel."' AND `server` = '".$this->serveraddress."'");
mysql_close($this->db);
if(mysql_num_rows($result) == 0)
{
...
mysql_close($this->db);
$this->database_connect();
mysql_query("INSERT INTO `".$this->database_name."`.`bot_vote` (`id` ,`user` ,`host` ,`channel` ,`server` ,`vote`)VALUES (NULL , '0', '0', '".$banchannel."', '".$banserver."', '0');");
mysql_close($this->db);
$this->database_connect();
$result = mysql_query("SELECT `id` FROM `bot_votekick` WHERE `user` = '".$banuser."' AND `host` = '".$host."' AND `channel` = '".$banchannel."' AND `server` = '".$banserver."' AND `active` = '1'");
mysql_close($this->db);
$user = mysql_fetch_assoc($result);
$this->votebanned[$this->serverchannel]['id'] = $user['id'];
...
function vote(){
$this->database_connect();
$result = mysql_query("SELECT `vote` FROM `bot_vote` WHERE `channel` = '".$this->serverchannel."' AND `server` = '".$this->serveraddress."'");
mysql_close($this->db);
if(mysql_num_rows($result) != 0)
{
if (!$this->data['message_target'])
...
$this->database_connect();
$this->send("INSERT INTO `".$this->database_name."`.`bot_banlist` (`id` ,`user` ,`host` ,`channel` ,`server` ,`time`)VALUES (NULL , '".$banuser."', '".$host."', '".$banchannel."', '".$banserver."', '".$bantimes."')");
mysql_close($this->db);
}
else
{
...
mysql_close($this->db);
$this->database_connect();
$result = mysql_query("SELECT `id` FROM `bot_voteban` WHERE `user` = '".$banuser."' AND `host` = '".$host."' AND `channel` = '".$banchannel."' AND `server` = '".$banserver."' AND `active` = '1'");
mysql_close($this->db);
$user = mysql_fetch_assoc($result);
$this->votebanned[$this->serverchannel]['id'] = $user['id'];
...
if($this->voteban[$this->serverchannel]['num'] != 0)
{
$this->database_connect();
...
$bot['database_user'] = "thenexus_bot";
$bot['database_password'] = "ds4342sdafds54";
$bot['database_name'] = "thenexus_bot";
